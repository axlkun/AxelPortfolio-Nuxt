<template>
    ...
    <v-sheet class="projects">
        <v-sheet class="container">
            <v-sheet class="title">
                <h1>Startups y SideProjects</h1>
                <p>Eres un desarrollador y buscas desarrollar SideProjects que se conviertan en Startups?  <br> Aquí puedes encontrar valiosos recursos que te ayudará en el camino.
                </p>
            </v-sheet>
        </v-sheet>
...
const getArticles = async () => {
        try {
        const response = await api.get('/api/articles/filter?seccion=startup');
